...
class TestDataBase(TestCase):
    ...
    def test_method_get_balance(self):
        """
        Check @staticmethod get_balance for several case:
    ...
    def test_auto_payment_after_apply_make_order_false(self):
        """
        Check auto payment after applying make_order()
        2. There is not a required amount
        """
        Order.objects.all().delete()
        cart = Order.get_cart(self.user)
        OrderItem.objects.create(order=cart, product=self.pr, price=2, quantity=10)
    ...
    def test_auto_payment_for_earlier_order(self):
        ...
        After creating new order = 1000 applying payment = 1000:
            ...
            - balance must become 13000 + 1000 - 13556 = 444
        """
        cart = Order.get_cart(self.user)
        OrderItem.objects.create(order=cart, product=self.pr, price=2, quantity=500)
